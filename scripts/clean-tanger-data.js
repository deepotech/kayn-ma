...
const inputFile = path.join(__dirname, '../tanger.json');
const outputFile = path.join(__dirname, '../src/data/tanger.json');
...
for (const item of rawData) {
    ...
    if (
        ...
        if (
          ...
          normalizedCategory = CAR_RENTAL_AR; // Default fallback to rental
        }
    } else {
        if (
            catText.includes('مستعملة') || 
...
console.log(`Successfully cleaned tanger data. Original: ${rawData.length}, Cleaned: ${cleaned.length}`);
