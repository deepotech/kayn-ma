...
const rawFile = path.join(__dirname, '../El Kelâa des Sraghna.json');
const cleanedFile = path.join(__dirname, '../src/data/kelaat-sraghna.json');
...
function runValidation() {
    console.log("Starting Kelaat Sraghna Data Validation...");
    ...
    cleanedData.forEach(item => {
        const slug = `${slugify(item.title)}-kelaat-sraghna`;
        if (slugs.has(slug)) {
            duplicateSlugs++;
    ...
    // 6. Verify city slug is exactly kelaat-sraghna
    const expectedCitySlug = 'kelaat-sraghna';
    ...
    console.log(`\n==================================================`);
    console.log(`📋 VALIDATION REPORT FOR KELAAT SRAGHNA`);
    console.log(`==================================================`);
