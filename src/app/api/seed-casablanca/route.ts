...
const CATEGORY_MAP: Record<string, string> = {
    ...
    'location de voitures': 'Car Rental Agency',
    'louer une voiture': 'Car Rental Agency',

    'concessionnaire': 'Used Car Dealer',
    'vente de voitures': 'Used Car Dealer',
    'voitures occasion': 'Used Car Dealer',
};
...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const dataPath = path.join(process.cwd(), 'src', 'data', 'casablanca.json');

        if (!fs.existsSync(dataPath)) {
            return NextResponse.json({
                success: false,
                error: 'casablanca.json not found in src/data/'
            }, { status: 404 });
        }
        ...
        for (const agency of rawData) {
            ...
            agencies.push({
                ...
                slug: generateSlug(agency.title, 'casablanca'),
                city: 'casablanca',
        ...
        if (clearExisting) {
            const deleted = await RentAgency.deleteMany({ city: 'casablanca' });
            console.log(`[Seed Casablanca] Cleared ${deleted.deletedCount} existing Casablanca agencies`);
        }
        ...
        return NextResponse.json({
            ...
            message: `Seeded ${agencies.length} Casablanca agencies successfully!`
        });

    } catch (error) {
        console.error('[Seed Casablanca] Error:', error);
        return NextResponse.json({
            success: false,
